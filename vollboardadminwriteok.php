...
$sql ="SELECT * FROM vollboard  WHERE serial='$serial'";
  ...
  if($_GET['type'] == "modi"){
      if($row['pw'] == $pw){
          $query5="update vollboard set subject = '$subject', content= '$content', writer='$writer' where serial=$serial";
       }else {
          echo "<script> 
   ...
   exit; 
     ...
     // $query5="delete from vollboard where serial=$serial";
       ...
       $query5="insert into vollsubject (subject, area1, area2, area3, electrator, writer, pw, writetime, active) values  ('$subject', '$area1', '$area2', '$area3','$electrator', '$writer', '$pw', now(), 0) ";
  }
